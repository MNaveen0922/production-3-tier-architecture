...
SSM paths (created by Terraform modules/ssm/):
  /support-desk/prod/config/rds_endpoint      → DB_HOST
  /support-desk/prod/config/rds_db_name       → DB_NAME
  /support-desk/prod/config/rds_db_user       → DB_USER
  /support-desk/prod/config/rds_secret_arn    → used to fetch DB password
  /support-desk/prod/config/orders_queue_url  → SQS_QUEUE_URL
  /support-desk/prod/config/sns_topic_arn     → SNS_TOPIC_ARN
  /support-desk/prod/config/assets_bucket     → S3_BUCKET
...
def load_config() -> dict:
    ...
    try:
        ...
        # e.g. /support-desk/prod/config/rds_endpoint → rds_endpoint
        params = {
